...
spotify_mysterious_sample = spotify_population[
    spotify_population["acousticness"] > 0.95
]
...
attrition_strat = attrition_pop.groupby("RelationshipSatisfaction").sample(
    frac=0.25, random_state=2022
)
...
attrition_clust = attrition_clust_prep.groupby("RelationshipSatisfaction").sample(
    n=len(attrition_pop) // 4, random_state=2022
)
...
dice["mean_roll"] = (
    dice["die1"] + dice["die2"] + dice["die3"] + dice["die4"] + dice["die5"]
) / 5
...
sampling_distribution_5 = [
    attrition_pop.sample(n=5)["Attrition"].mean() for _ in range(1000)
]
sampling_distribution_50 = [
    attrition_pop.sample(n=50)["Attrition"].mean() for _ in range(1000)
]
sampling_distribution_500 = [
    attrition_pop.sample(n=500)["Attrition"].mean() for _ in range(1000)
]
...
sampling_distribution = [
    spotify_population.sample(n=500)["popularity"].mean() for _ in range(5000)
]
bootstrap_distribution = [
    spotify_sample.sample(n=500, replace=True)["popularity"].mean() for _ in range(5000)
]
...
sampling_distribution = [
    spotify_population.sample(n=5000)["popularity"].mean() for _ in range(5000)
]
bootstrap_distribution = [
